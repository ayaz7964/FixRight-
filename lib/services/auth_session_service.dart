...
class AuthSessionService {
      ...
      try {
        ...
        if (e.code == 'user-not-found') {
          ...
          try {
            ...
            print('✅ Firebase Auth account created and signed in for: $emailAlias');
            return userCredential;
          } catch (createError) {
  ...
  /// 
  /// We need to also update it in Firebase Auth for consistency.
  Future<void> updatePassword({
    ...
    try {
      ...
      // First, sign in with the new password (caller must have verified OTP)
      // Get current user - if they're not signed in, this will fail
      final user = _auth.currentUser;

      if (user != null) {
        // User is already signed in, update their password directly
        await user.updatePassword(newPassword);
        print('✅ Firebase Auth password updated');
      } else {
        // User is not signed in - create account if not exists
        // This can happen if user resets password without being logged in
        try {
          await _auth.createUserWithEmailAndPassword(
            email: emailAlias,
            password: newPassword,
          );
          print('✅ Firebase Auth account created with new password');
        } on FirebaseAuthException catch (e) {
          if (e.code == 'email-already-in-use') {
            // Account exists - user should be signed in first
            print(
                'Note: User account exists, they should sign in with new password');
          } else {
            rethrow;
          }
        }
      ...
      print('Firebase Auth Password Update Error: ${e.code} - ${e.message}');
      throw _handleAuthException(e);
    } catch (e) {
      print('Error updating Firebase Auth password: $e');
      throw Exception('Failed to update session password: $e');
    }
  }
  ...
  Future<void> sendPasswordResetEmail({
    ...
    try {
      ...
      print(
          'Firebase Auth Password Reset Error: ${e.code} - ${e.message}');
      throw _handleAuthException(e);
    } catch (e) {
