...
// ─────────────────────────────────────────────────────────────
//  SharedPreferences keys (must match LoginPage._PrefKeys)
// ─────────────────────────────────────────────────────────────
class _PrefKeys {
  static const String isLoggedIn    = 'is_logged_in';
  static const String savedPhone    = 'saved_phone';
  static const String savedPassword = 'saved_password';
  static const String savedPhoneCode= 'saved_phone_code';
...
class AuthService {
  ...
  Future<void> createUserProfile({
    ...
    try {
      ...
      payload.putIfAbsent('latitude', () => null);
      payload.putIfAbsent('longitude', () => null);
      payload.putIfAbsent('liveLocation', () => {});
      payload.putIfAbsent('profileImage', () => '');
      payload.putIfAbsent('profileUrl', () => '');
  ...
  /// Fetch and return the plain-text password for a given phone number.
  /// Used by ForgotPasswordPage after OTP verification.
  Future<String> showPassword({required String phoneNumber}) async {
    try {
      final authDoc =
          await _firestore.collection('auth').doc(phoneNumber).get();
      ...
      final authData = authDoc.data() as Map<String, dynamic>;
      final password = authData['password'] as String? ?? 'Password not set';

      print('Password fetched for: $phoneNumber');
      return password;
    } catch (e) {
      print('Error fetching password: $e');
      rethrow;
  ...
  // ═══════════════════════════════════════════════════════════
  // SIGN OUT  ←  clearSession is now merged here
  // ═══════════════════════════════════════════════════════════

  /// Signs out the user completely:
  ///   1. Sets presence offline
  ///   2. Signs out of Firebase Auth session
  ///   3. Clears all SharedPreferences login data
  ///
  /// Usage (anywhere in the app):
  ///   await _authService.signOut();
  ///   Navigator.pushReplacementNamed(context, '/login');
  Future<void> signOut() async {
    // Step 1: Update presence to offline
    try {
      final presenceService = UserPresenceService();
      await presenceService.setOfflineBeforeLogout();
    } catch (e) {
    ...
    // Step 2: Sign out from AuthSessionService + FirebaseAuth
    try {
      final sessionService = AuthSessionService();
      await sessionService.signOut();
      await _auth.signOut();
    ...
    // Step 3: Clear all SharedPreferences login data  ← NEW
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool(_PrefKeys.isLoggedIn, false);
      await prefs.setBool(_PrefKeys.rememberMe, false);
      await prefs.remove(_PrefKeys.savedPhone);
      await prefs.remove(_PrefKeys.savedPassword);
      await prefs.remove(_PrefKeys.savedPhoneCode);
