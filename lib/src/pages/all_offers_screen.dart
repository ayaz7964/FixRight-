...
// ══════════════════════════════════════════════════════════════
//  ALL OFFERS SCREEN  —  v5 (UI rewrite + TranslatedText)
// ══════════════════════════════════════════════════════════════
class AllOffersScreen extends StatefulWidget {
  final String? phoneUID;
  final String buyerCity;
...
class _AllOffersScreenState extends State<AllOffersScreen> {
  ...
  static const _cats = [
    ...
    'AC Repair','Painting','Mechanic','Roofing','Gardening',
    'Welding','Masonry','Tiling',
  ];
  static const _catEmoji = {
    'All':'🌟','Plumbing':'🔧','Electrical':'⚡','Cleaning':'🧹',
    'Carpentry':'🪚','AC Repair':'❄️','Painting':'🎨','Mechanic':'🔩',
    'Roofing':'🏗️','Gardening':'🌿','Welding':'🔥','Masonry':'🧱','Tiling':'🪟',
  };
  ...
  Future<void> _onData(QuerySnapshot snap) async {
    ...
    if (mounted) setState(() { _allOffers = enriched; _loading = false; });
  }

  // ── Filter + sort — ALL LOGIC UNCHANGED ──────────────────
  List<Map<String, dynamic>> get _visible {
    var list = List<Map<String, dynamic>>.from(_allOffers);
    ...
    if (!_nearMe && widget.buyerCity.isNotEmpty) {
      final bc = widget.buyerCity.trim().toLowerCase();
      list.sort((a, b) {
        final aL = (a['sellerCity'] as String? ?? '').toLowerCase() == bc ? 0 : 1;
        final bL = (b['sellerCity'] as String? ?? '').toLowerCase() == bc ? 0 : 1;
        return aL.compareTo(bL);
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Service Offers',
            style: TextStyle(color: Colors.white, fontSize: 20,
                fontWeight: FontWeight.w900, letterSpacing: -0.3)),
        actions: [
          IconButton(icon: const Icon(Icons.sort_rounded, color: Colors.white, size: 22), onPressed: _showSort),
          const GlobalLanguageButton(color: Colors.white),
          const SizedBox(width: 4),
        ],
      ),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(
          widget.buyerCity.isNotEmpty
              ? '${_allOffers.length} offers • ${widget.buyerCity}'
              : '${_allOffers.length} offers available',
          style: TextStyle(color: Colors.white.withOpacity(0.80), fontSize: 12.5, fontWeight: FontWeight.w500),
        ),
        const SizedBox(height: 10),
        Container(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: _query.isNotEmpty
                  ? IconButton(icon: const Icon(Icons.close_rounded, size: 17, color: Colors.grey), onPressed: _searchCtrl.clear)
                  : null,
              border: InputBorder.none,
        ...
        if (widget.buyerCity.isNotEmpty) ...[
          ...
          Row(children: [
            _pill('All', !_nearMe, () => setState(() => _nearMe = false)),
            const SizedBox(width: 8),
            _pill('📍 ${widget.buyerCity}', _nearMe, () => setState(() => _nearMe = true)),
          ]),
        ],
      ]),
    );
  ...
  Widget _pill(String l, bool active, VoidCallback onTap) => GestureDetector(
    ...
    child: Container(
      ...
      child: Text(l, style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: active ? _teal : Colors.white)),
    ),
  );

  // ── Category chips ────────────────────────────────────────
  Widget _buildCats() => Container(
    color: Colors.white, height: 52,
    child: ListView.builder(
      scrollDirection: Axis.horizontal,
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
      itemCount: _cats.length,
    ...
    child: Row(children: [
      ...
      GestureDetector(
        ...
        child: Container(
          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
          decoration: BoxDecoration(
              color: _teal.withOpacity(0.08), borderRadius: BorderRadius.circular(14),
              border: Border.all(color: _teal.withOpacity(0.2))),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            const Icon(Icons.swap_vert_rounded, size: 14, color: _teal),
            const SizedBox(width: 4),
            Text(
              _sort == 'price_asc' ? 'Price ↑' : _sort == 'price_desc' ? 'Price ↓' : 'Top Rated',
        ...
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(width: 70, height: 70,
              ...
              child: const Icon(Icons.storefront_outlined, size: 36, color: _teal)),
            const SizedBox(height: 16),
            Text(
              _query.isNotEmpty ? 'No results for "$_query"'
                  : _nearMe ? 'No offers near ${widget.buyerCity}'
                  : _cat != null ? 'No "$_cat" offers yet' : 'No active offers yet',
  ...
  Widget _sortTile(String l, String v) => ListTile(
    ...
    trailing: _sort == v ? const Icon(Icons.check_rounded, color: _teal) : null,
    onTap: () { setState(() => _sort = v); Navigator.pop(context); },
  );
}
  ...
  @override Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (_, __) => Container(
        height: 210, margin: const EdgeInsets.only(bottom: 14),
        decoration: BoxDecoration(
...
// ══════════════════════════════════════════════════════════════
//  OFFER CARD  — improved UI + TranslatedText description
// ══════════════════════════════════════════════════════════════
class _OfferCard extends StatelessWidget {
  final Map<String, dynamic> offerData;
  final String offerId, buyerUid, buyerCity;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: Colors.black.withOpacity(0.07), blurRadius: 16, offset: const Offset(0, 5)),
        ],
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (imageUrl.isNotEmpty)
          Stack(children: [
            ...
            Positioned(top: 12, right: 12, child: _priceBadge(price)),
            // Near-me badge on image
            if (nearBuyer) Positioned(top: 12, left: 12, child: _nearBadge()),
          ])
        else
          // No image → coloured gradient header
          Container(
            height: 72,
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [_teal.withOpacity(0.85), _tealDark],
                begin: Alignment.topLeft, end: Alignment.bottomRight,
              ),
              borderRadius: const BorderRadius.vertical(top: Radius.circular(22)),
            ),
            ...
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(
                child: Text(title,
                    ...
                    style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 15, height: 1.3)),
              ),
              const SizedBox(width: 10),
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: [
                  if (sellerCity.isNotEmpty) ...[
                    ...
                    Flexible(child: Text(sellerCity, style: TextStyle(fontSize: 11, color: Colors.grey[600]), maxLines: 1, overflow: TextOverflow.ellipsis)),
                    const SizedBox(width: 6),
                  ],
                  if (nearBuyer) _nearBadge(small: true),
                ]),
              ])),
              // Rating + orders column
                ...
                if (rating > 0)
                  Row(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Text(rating.toStringAsFixed(1),
                        style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: Colors.grey[700])),
                  ]),
                if (orders > 0)
                  Text('$orders orders', style: TextStyle(fontSize: 10, color: Colors.grey[400])),
              ]),
            ]),

            const SizedBox(height: 12),

            // ── Title (if image present) ────────────────────
            if (imageUrl.isNotEmpty) ...[
              Text(title, maxLines: 2, overflow: TextOverflow.ellipsis,
            ...
            if (desc.isEmpty)
              SpeakButton(
                text: title,
                contentId: 'offer_speak_$offerId',
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: const [
                    ...
                    SizedBox(width: 6),
                    Text('Place Order', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 13)),
                  ]),
                ),
              ),
            ]),
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.end, mainAxisSize: MainAxisSize.min, children: [
      Text('PKR ${price.toStringAsFixed(0)}',
          style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900,
              color: light ? _tealDark : Colors.white)),
  ...
  Widget _nearBadge({bool small = false}) => Container(
    padding: EdgeInsets.symmetric(horizontal: small ? 7 : 10, vertical: small ? 2 : 5),
    decoration: BoxDecoration(
      color: _teal, borderRadius: BorderRadius.circular(10),
      boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.15), blurRadius: 4)],
    ),
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      Icon(Icons.near_me_rounded, size: small ? 10 : 12, color: Colors.white),
      const SizedBox(width: 3),
      Text('Near You', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: small ? 9 : 11)),
    ]),
  );
}