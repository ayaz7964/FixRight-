...
class _OfferPlaceOrderSheetState extends State<OfferPlaceOrderSheet> {
  ...
  double get _insuranceAmt => _wantsInsurance ? (_price * kOfferInsuranceRate) : 0;
  ...
  Future<void> _loadSellerInfo() async {
    try {
      ...
      try {
        final cfg = await FirebaseFirestore.instance.collection('config').doc('commission').get();
        if (cfg.exists) _commissionRate = (cfg.data()?['rate'] ?? 0.10).toDouble();
      } catch (_) {}
    } catch (_) {}
  ...
  void dispose() {
    ...
    _timingCtrl.dispose(); _notesCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _placeOrder() async {
    ...
    try {
      ...
      batch.set(jobRef, {
        ...
        'budget': _price, 'acceptedAmount': _price,
        'orderType': _wantsInsurance ? 'insured' : 'simple',
        'insuranceAmount': _insuranceAmt, 'totalAmount': _totalAmt,
        'status': newStatus,
        'paymentStatus': _wantsInsurance ? 'pending_payment' : 'cash_on_delivery',
        'postedBy': widget.buyerUid, 'posterName': buyerName, 'posterImage': buyerImg,
        'acceptedBidder': _sellerId, 'sellerName': sellerName,
        'location': _addrCtrl.text.trim(),
        'city': _cityCtrl.text.trim(),
        'timing': _timingCtrl.text.trim(),
        'notes': _notesCtrl.text.trim(),
        'bidsCount': 0, 'insuranceClaimed': false, 'insuranceClaimCount': 0,
        'claimDeadline': _wantsInsurance ? null : Timestamp.fromDate(claimDeadline),
      ...
      // ── Seller order mirror ─────────────────────────────────
      batch.set(db.collection('sellers').doc(_sellerId).collection('orders').doc(jobRef.id), {
        'orderId': jobRef.id, 'jobId': jobRef.id, 'jobTitle': title, 'description': desc,
        'skills': skills, 'buyerId': widget.buyerUid, 'buyerName': buyerName,
        'sellerId': _sellerId, 'sellerName': sellerName,
        'proposedAmount': _price, 'commissionDeducted': _commission,
        'commissionRate': _commissionRate, 'isFreeOrder': _isFreeOrder,
        'orderType': _wantsInsurance ? 'insured' : 'simple',
        'insuranceAmount': _insuranceAmt, 'totalAmount': _totalAmt,
        'status': newStatus, 'paymentStatus': _wantsInsurance ? 'pending_payment' : 'cash_on_delivery',
        'insuranceClaimed': false, 'orderSource': 'offer', 'sourceOfferId': widget.offerId,
        'createdAt': FieldValue.serverTimestamp(), 'updatedAt': FieldValue.serverTimestamp(),
      });

      // ── Increment offer orders count ────────────────────────
      batch.update(db.collection('sellers').doc(_sellerId).collection('offers').doc(widget.offerId),
      ...
      if (!(await db.collection('conversations').doc(convId).get()).exists) {
        await db.collection('conversations').doc(convId).set({
          ...
          'participantNames': {widget.buyerUid: buyerName, _sellerId: sellerName},
          'participantRoles': {widget.buyerUid: 'buyer', _sellerId: 'seller'},
          'participantProfileImages': {widget.buyerUid: buyerImg, _sellerId: sellerImg},
          'lastMessage': "Order placed! Let's get started.",
          'lastMessageAt': Timestamp.now(), 'createdAt': Timestamp.now(),
          'unreadCounts': {widget.buyerUid: 0, _sellerId: 1},
          'relatedJobId': jobRef.id, 'relatedJobTitle': title,
        });
      }
      ...
      await NotificationService.send(
        ...
        body: '$buyerName placed an order for "$title". ${_wantsInsurance ? 'Insured — awaiting payment.' : 'Cash on delivery.'}',
        type: 'bid_accepted', jobId: jobRef.id, relatedUserName: buyerName,
      );

      if (!mounted) return;
      Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(_wantsInsurance
  ...
  Widget build(BuildContext context) {
    ...
    return DraggableScrollableSheet(
      ...
      builder: (ctx, sc) => Container(
        ...
        child: Form(key: _formKey, child: ListView(controller: sc,
          padding: const EdgeInsets.fromLTRB(20, 10, 20, 30), children: [
          // Drag handle
          Center(child: Container(width: 42, height: 4, margin: const EdgeInsets.only(bottom: 18),
            decoration: BoxDecoration(color: Colors.grey[300], borderRadius: BorderRadius.circular(2)))),

          // ── Offer summary ────────────────────────────────────
          Container(padding: const EdgeInsets.all(14),
            decoration: BoxDecoration(color: _teal.withOpacity(0.05), borderRadius: BorderRadius.circular(16),
              border: Border.all(color: _teal.withOpacity(0.15))),
            child: Row(children: [
              CircleAvatar(radius: 24, backgroundColor: _teal.withOpacity(0.12),
                backgroundImage: sellerImg.isNotEmpty ? NetworkImage(sellerImg) : null,
                child: sellerImg.isEmpty ? Text(sellerName[0].toUpperCase(),
                  style: const TextStyle(color: _teal, fontWeight: FontWeight.bold, fontSize: 20)) : null),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(title, style: const TextStyle(fontSize: 15.5, fontWeight: FontWeight.w800, color: Colors.black87),
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                Text('PKR ${_price.toStringAsFixed(0)}',
                  style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: _teal)),
                Text('base price', style: TextStyle(fontSize: 10, color: Colors.grey[400])),
              ]),
            ])),
          const SizedBox(height: 6),
          if (skills.isNotEmpty) Wrap(spacing: 6, runSpacing: 4,
          ...
          // ── Commission info ──────────────────────────────────
          if (_loadingInfo) const LinearProgressIndicator(color: _teal, minHeight: 2)
          else _infoBanner(
            _isFreeOrder
          ...
          _textField(_addrCtrl, 'Full address (street, area)', Icons.location_on_outlined,
            validator: (v) => (v ?? '').trim().isEmpty ? 'Address is required' : null),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _textField(_cityCtrl, 'City', Icons.location_city_outlined,
          ...
          const SizedBox(height: 8),
          _textField(_timingCtrl, 'e.g. Tomorrow 10am, Any weekday morning', Icons.schedule_outlined,
            validator: (v) => (v ?? '').trim().isEmpty ? 'Please enter preferred timing' : null),
          const SizedBox(height: 20),
          ...
          TextFormField(controller: _notesCtrl, maxLines: 2,
            decoration: _inputDeco('Any special requirements, access info…', Icons.notes_outlined)),
          const SizedBox(height: 24),

          // ── Insurance toggle ─────────────────────────────────
          Container(
            decoration: BoxDecoration(
              color: _wantsInsurance ? Colors.blue.shade50 : Colors.grey.shade50,
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: _wantsInsurance ? Colors.blue.shade300 : Colors.grey.shade200)),
            child: Column(children: [
              ...
              if (_wantsInsurance) Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  const SizedBox(height: 12),
                  _summaryRow('Insurance Fee (20%)', 'PKR ${_insuranceAmt.toStringAsFixed(0)}', Colors.blue),
                  const SizedBox(height: 4),
                  _summaryRow('Total to Transfer', 'PKR ${_totalAmt.toStringAsFixed(0)}', Colors.blue.shade700, bold: true),
                  ...
                  Container(padding: const EdgeInsets.all(10),
                    ...
                    child: Text(
                      ...
                      style: TextStyle(fontSize: 11.5, color: Colors.orange.shade800, height: 1.4))),
                ])),
            ])),
          const SizedBox(height: 14),

          // Cash note
          if (!_wantsInsurance) Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Text('💵 Cash on Delivery — Pay PKR ${_price.toStringAsFixed(0)} directly to the worker after job completion.',
                style: TextStyle(fontSize: 12.5, color: Colors.green.shade800, height: 1.4))),
            ])),
          ...
          Container(padding: const EdgeInsets.all(16),
            ...
            child: Column(children: [
              _summaryRow('Service Price', 'PKR ${_price.toStringAsFixed(0)}', Colors.black87),
              const SizedBox(height: 6),
              _summaryRow(
                _isFreeOrder ? 'Commission' : 'Commission (${(_commissionRate * 100).toStringAsFixed(0)}%)',
                _isFreeOrder ? 'FREE ($_freeLeft left)' : 'PKR ${_commission.toStringAsFixed(0)} from seller',
                _isFreeOrder ? Colors.green : Colors.orange),
              if (_wantsInsurance) ...[
                const SizedBox(height: 6),
                _summaryRow('Insurance (20%)', 'PKR ${_insuranceAmt.toStringAsFixed(0)}', Colors.blue),
              ],
          ...
          SizedBox(width: double.infinity,
            child: ElevatedButton.icon(
              ...
              label: Text(
                _isPlacing ? 'Placing Order…'
                    : _wantsInsurance ? 'Place Insured Order  •  PKR ${_totalAmt.toStringAsFixed(0)}'
                    : 'Place Order  •  Cash on Delivery',
                style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
              style: ElevatedButton.styleFrom(
                backgroundColor: _wantsInsurance ? Colors.blue.shade700 : _teal,
  ...
  Widget _infoBanner(String text, Color color) => Container(
    padding: const EdgeInsets.all(13),
    decoration: BoxDecoration(color: color.withOpacity(0.08), borderRadius: BorderRadius.circular(13),
  ...
  Widget _summaryRow(String l, String v, Color color, {bool bold = false}) => Row(
    mainAxisAlignment: MainAxisAlignment.spaceBetween,
    children: [
      Text(l, style: TextStyle(fontSize: 13, color: Colors.grey[700])),
      Text(v, style: TextStyle(fontSize: 13, color: color, fontWeight: bold ? FontWeight.w800 : FontWeight.w600)),
    ]);
}