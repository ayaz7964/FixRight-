...
class _BuyerOrdersPageState extends State<BuyerOrdersPage>
  ...
  void initState() {
    super.initState();
    // 4 tabs: Open | In Progress | Completed | History
...
// ─────────────────────────────────────────────────────────────
// In Progress List — includes pending_payment and payment_submitted
// ─────────────────────────────────────────────────────────────
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final docs = (snapshot.data?.docs ?? []).where((d) {
          ...
          return s == 'in_progress' || s == 'pending_payment' || s == 'payment_submitted' || s == 'claim_pending' || s == 'expert_assigned';
        }).toList()
          ..sort((a, b) {
...
// ─────────────────────────────────────────────────────────────
// Insurance Payment Section — receipt upload
// ─────────────────────────────────────────────────────────────
  ...
  Future<void> _pickAndUploadReceipt() async {
    ...
    try {
      ...
      if (url != null && url.isNotEmpty) {
        ...
        // Notify company / admin (use a fixed admin uid or a collection)
        ...
        if (sellerId.isNotEmpty) {
          await NotificationService.send(toUid: sellerId, title: 'Payment Submitted', body: 'Buyer submitted payment receipt. Waiting admin verification.', type: 'payment_verified', jobId: widget.jobId);
        }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Icon(Icons.pending_actions, color: Colors.orange.shade700),
          const SizedBox(width: 8),
          Expanded(child: Text('Payment Required to Activate Order', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.orange.shade800, fontSize: 15))),
        ]),
        const SizedBox(height: 10),
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              const SizedBox(width: 8),
              Text('Upload Payment Receipt', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue.shade700, fontSize: 14)),
            ]),
            const SizedBox(height: 8),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (receiptUrl != null && receiptUrl.isNotEmpty) ...[
          const SizedBox(height: 12),
          ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.network(receiptUrl, height: 120, width: double.infinity, fit: BoxFit.cover)),
        ],
      ]),
    );
  }
...
// ─────────────────────────────────────────────────────────────
// Insurance Active Banner (job in_progress + insured)
// ─────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 8),
        Expanded(child: Text('Insured Order — Your payment is held by company until job is completed and you approve it.', style: TextStyle(fontSize: 12, color: Colors.blue.shade800))),
      ]),
    );
  }
}
...
// ─────────────────────────────────────────────────────────────
// Completed Job Section — accept or claim insurance
// ─────────────────────────────────────────────────────────────
...
class _CompletedJobSectionState extends State<_CompletedJobSection> {
  ...
  Future<void> _acceptAndRelease() async {
    ...
    try {
      ...
      if (sellerId.isNotEmpty && _isInsured) {
        batch.update(FirebaseFirestore.instance.collection('sellers').doc(sellerId), {
          'Available_Balance': FieldValue.increment(acceptedAmount),
          'Earning': FieldValue.increment(acceptedAmount),
        });
        await NotificationService.send(toUid: sellerId, title: '💰 Earnings Released!', body: 'Buyer accepted the job. PKR ${acceptedAmount.toStringAsFixed(0)} has been added to your wallet.', type: 'earnings_released', jobId: widget.jobId);
      }
      await batch.commit();
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('✅ Job accepted! Payment released to seller.'), backgroundColor: Colors.green));
  ...
  Future<void> _fileClaim() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('jobs').doc(widget.jobId).update({
        'status': 'claim_pending', 'insuranceClaimed': true, 'insuranceClaimCount': claimCount + 1,
        'claimFiledAt': FieldValue.serverTimestamp(), 'claimStatus': 'pending_review', 'updatedAt': FieldValue.serverTimestamp(),
      });
      ...
      if (sellerId.isNotEmpty) await NotificationService.send(toUid: sellerId, title: '⚠️ Insurance Claim Filed', body: 'Buyer filed an insurance claim. You must revisit and complete the job.', type: 'claim_filed', jobId: widget.jobId);
  ...
  Widget build(BuildContext context) {
    if (_claimed) {
      return Container(
        ...
        child: Row(children: [
          Icon(Icons.verified, color: Colors.blue.shade700, size: 20), const SizedBox(width: 10),
          Expanded(child: Text('Insurance claim filed. Expert team has been notified.', style: TextStyle(fontWeight: FontWeight.w600, color: Colors.blue.shade800, fontSize: 13))),
        ]),
      );
    }

    if (widget.jobData['buyerAccepted'] == true) {
      return Container(
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.green.shade200)),
        child: Row(children: [
          Icon(Icons.check_circle, color: Colors.green.shade700, size: 20), const SizedBox(width: 10),
          Expanded(child: Text('Job completed and accepted. Payment released to seller.', style: TextStyle(fontWeight: FontWeight.w600, color: Colors.green.shade800, fontSize: 13))),
        ]),
      );
    }
    ...
    if (!_isInsured) {
      return Container(
        ...
        decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.green.shade200)),
        child: Row(children: [
          Icon(Icons.task_alt, color: Colors.green.shade700, size: 20), const SizedBox(width: 10),
          Expanded(child: Text('Job completed. Cash payment should have been made to the worker.', style: TextStyle(color: Colors.green.shade800, fontSize: 13))),
        ]),
      );
    }

    // Insured + can claim or accept
    final deadline = widget.jobData['claimDeadline'] as Timestamp?;
    ...
    return Column(children: [
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Icon(Icons.task_alt, color: Colors.green.shade700, size: 18), const SizedBox(width: 8),
            Text('Job marked complete by seller', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green.shade700)),
          ]),
          const SizedBox(height: 8),
      ...
      if (_canClaim) ...[
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Icon(Icons.shield_outlined, color: Colors.orange.shade700, size: 18), const SizedBox(width: 8),
              Text('Insurance Window — $daysLeft day(s) left', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.orange.shade800)),
            ]),
            const SizedBox(height: 8),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Expanded(child: Text('Expert Assigned', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.purple.shade800, fontSize: 15))),
        ]),
        const SizedBox(height: 10),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Expanded(child: Text('Insurance Claim Filed', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red.shade800, fontSize: 15))),
        ]),
        const SizedBox(height: 10),
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snap) {
        ...
        final inProgress = docs.where((d) { final s = (d.data() as Map)['status']; return s == 'in_progress'; }).length;
        ...
        return ListView(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: [
                  ...
                  const SizedBox(width: 8),
                  Text('Total Spent: PKR ${totalSpent.toStringAsFixed(0)}', style: const TextStyle(color: Colors.white70, fontSize: 13)),
                ]),
              ]),
            ),
            ...
            ...docs.map((doc) {
              ...
              return _HistoryJobCard(jobData: data, jobId: doc.id);
            }),
          ],
        );
      },
...
class _HistoryJobCard extends StatelessWidget {
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (location.isNotEmpty)
            Row(children: [Icon(Icons.location_on, size: 12, color: Colors.grey[500]), const SizedBox(width: 4), Expanded(child: Text(location, style: TextStyle(fontSize: 12, color: Colors.grey[600]), maxLines: 1, overflow: TextOverflow.ellipsis))]),
            ...
            FutureBuilder<DocumentSnapshot>(
              ...
              builder: (ctx, snap) {
                ...
                return Row(children: [
                  ...
                  const SizedBox(width: 4),
                  Text('Worker: $name', style: const TextStyle(fontSize: 12, color: Colors.black54)),
                ]);
              },
            ),
          ],
  ...
  Widget _infoChip(IconData icon, String label, Color color) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 12, color: color), const SizedBox(width: 4), Text(label, style: TextStyle(fontSize: 11, color: color, fontWeight: FontWeight.w600))]),
  );

  Widget _dateChip(String label, DateTime dt) => Row(children: [
    Icon(Icons.calendar_today, size: 11, color: Colors.grey[500]),
    const SizedBox(width: 3),
    Text('$label: ${DateFormat('dd MMM yy').format(dt)}', style: TextStyle(fontSize: 11, color: Colors.grey[600])),
  ]);

  Color _statusColor(String s) {
    switch (s) { case 'completed': case 'expert_completed': return Colors.green; case 'in_progress': return Colors.blue; case 'cancelled': return Colors.red; case 'pending_payment': case 'payment_submitted': return Colors.orange; default: return Colors.grey; }
  }
}
        ...
        if (!snap.hasData || snap.data!.docs.isEmpty) {
          return Container(
            ...
            child: Column(children: [
              Icon(Icons.gavel, size: 48, color: Colors.grey[300]),
              const SizedBox(height: 12),
              const Text('No bids yet', style: TextStyle(color: Colors.grey, fontSize: 16)),
              const SizedBox(height: 4),
              Text('Sellers will bid shortly', style: TextStyle(color: Colors.grey[400], fontSize: 12), textAlign: TextAlign.center),
            ]),
          );
        }
        return ListView.builder(
...
class _BidCardState extends State<_BidCard> {
  ...
  Future<void> _openConversation(BuildContext ctx) async {
    ...
    if (!convDoc.exists) {
      ...
      await db.collection('conversations').doc(convId).set({
        'participantIds': [widget.buyerUid, sellerId], 'participantNames': {widget.buyerUid: buyerName, sellerId: sellerName},
        'participantRoles': {widget.buyerUid: 'buyer', sellerId: 'seller'}, 'participantProfileImages': {widget.buyerUid: buyerData['profileImage'] ?? '', sellerId: sellerImage},
        'lastMessage': '', 'lastMessageAt': Timestamp.now(), 'createdAt': Timestamp.now(), 'unreadCounts': {widget.buyerUid: 0, sellerId: 0},
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isAccepted ? Colors.green.shade300 : isRejected ? Colors.red.shade100 : Colors.grey.shade200, width: isAccepted ? 2 : 1),
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 6)],
      ),
      child: Padding(
        padding: const EdgeInsets.all(14),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 10),
          if (proposal.isNotEmpty) Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.grey.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey.shade200)), child: Text(proposal, style: const TextStyle(fontSize: 13, height: 1.4))),
          if (skills.isNotEmpty) ...[
            const SizedBox(height: 8),
              ...
              builder: (ctx, snap) {
                ...
                return Row(children: [
                  Expanded(child: OutlinedButton.icon(
                    ...
                    icon: const Icon(Icons.message_outlined, size: 16), label: const Text('Contact'),
                    style: OutlinedButton.styleFrom(foregroundColor: Colors.teal, side: const BorderSide(color: Colors.teal), padding: const EdgeInsets.symmetric(vertical: 10), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                  )),
                  const SizedBox(width: 10),
...
class _OrderConfirmationSheetState extends State<_OrderConfirmationSheet> {
  ...
  Future<void> _placeOrder() async {
    ...
    try {
      ...
      // Fetch buyer's city for city filter
      final buyerDoc = await db.collection('users').doc(widget.buyerUid).get();
      final buyerData = buyerDoc.data() ?? {};
      final buyerCity = buyerData['city'] ?? '';

      batch.update(jobRef, {
        'status': newJobStatus, 'acceptedBidder': sellerId, 'acceptedAmount': _proposedAmount,
        'orderType': _wantsInsurance ? 'insured' : 'simple', 'insuranceAmount': _insuranceAmount, 'totalAmount': _totalAmount,
        'paymentStatus': _wantsInsurance ? 'pending_payment' : 'cash_on_delivery', 'insuranceClaimed': false, 'insuranceClaimCount': 0,
        'claimDeadline': _wantsInsurance ? null : Timestamp.fromDate(claimDeadline), 'city': buyerCity, 'updatedAt': FieldValue.serverTimestamp(),
      });
      ...
      batch.set(orderRef, {
        'orderId': widget.jobId, 'jobId': widget.jobId, 'jobTitle': widget.jobData['title'] ?? '', 'jobDescription': widget.jobData['description'] ?? '',
        'jobLocation': widget.jobData['location'] ?? '', 'skills': widget.jobData['skills'] ?? [], 'buyerId': widget.buyerUid, 'buyerName': '',
        'sellerId': sellerId, 'sellerName': sellerName, 'proposedAmount': _proposedAmount, 'commissionDeducted': isFreeOrder ? 0 : commissionAmount,
        'isFreeOrder': isFreeOrder, 'orderType': _wantsInsurance ? 'insured' : 'simple', 'insuranceAmount': _insuranceAmount, 'totalAmount': _totalAmount,
        'status': newJobStatus, 'paymentStatus': _wantsInsurance ? 'pending_payment' : 'cash_on_delivery', 'insuranceClaimed': false,
      ...
      if (!convDoc.exists) {
        await db.collection('conversations').doc(convId).set({
          'participantIds': [widget.buyerUid, sellerId], 'participantNames': {widget.buyerUid: buyerName, sellerId: sellerName},
          'participantRoles': {widget.buyerUid: 'buyer', sellerId: 'seller'}, 'participantProfileImages': {widget.buyerUid: buyerImage, sellerId: sellerImage},
          'lastMessage': 'Order placed! Let\'s get started.', 'lastMessageAt': Timestamp.now(), 'createdAt': Timestamp.now(),
          'unreadCounts': {widget.buyerUid: 0, sellerId: 1}, 'relatedJobId': widget.jobId, 'relatedJobTitle': widget.jobData['title'] ?? '',
        });
      }
      ...
      for (final d in otherBids.docs) {
        ...
        if (rejectedSellerId.isNotEmpty) {
          await NotificationService.send(toUid: rejectedSellerId, title: 'Bid Not Selected', body: 'The buyer selected another seller for "${widget.jobData['title']}".', type: 'bid_rejected', jobId: widget.jobId);
        }
      }
    ...
    return Padding(
      ...
      child: Container(padding: const EdgeInsets.all(20), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Column(children: [
            SwitchListTile(
              ...
              activeColor: Colors.blue,
              title: const Text('Add Insurance', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
              subtitle: const Text('+20% — Guaranteed completion & 3-day claim window', style: TextStyle(fontSize: 11)),
              secondary: Icon(Icons.shield_outlined, color: _wantsInsurance ? Colors.blue : Colors.grey),
            ),
            if (_wantsInsurance) Padding(
        ...
        if (!_wantsInsurance) Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.green.shade200)),
          child: Row(children: [
            Icon(Icons.payments_outlined, color: Colors.green.shade700, size: 16), const SizedBox(width: 8),
            Expanded(child: Text('Pay PKR ${_proposedAmount.toStringAsFixed(0)} in cash after job completion.', style: TextStyle(fontSize: 11, color: Colors.green.shade800))),
          ])),
        const SizedBox(height: 20),
        SizedBox(width: double.infinity, child: ElevatedButton.icon(
          onPressed: _isPlacing ? null : _placeOrder,
  ...
  Widget _sheetRow(String label, String value, {Color? color, bool bold = false}) => Padding(
    padding: const EdgeInsets.symmetric(vertical: 4),
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
...
class _JobSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(elevation: 2, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 8),
        if ((jobData['description'] ?? '').isNotEmpty) Text(jobData['description'], style: TextStyle(color: Colors.grey[600], fontSize: 14, height: 1.4)),
        const SizedBox(height: 12),
  ...
  Widget _infoRow(IconData icon, String text) => Row(children: [
    Icon(icon, size: 14, color: Colors.grey[500]), const SizedBox(width: 6),
    ...
    return FutureBuilder<DocumentSnapshot>(
      ...
      builder: (context, snap) {
        ...
        return Card(elevation: 2, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: Padding(
          padding: const EdgeInsets.all(16),
          child: Row(children: [
            CircleAvatar(radius: 30, backgroundColor: Colors.teal.shade100, backgroundImage: image != null && image.isNotEmpty ? NetworkImage(image) : null,
              child: image == null || image.isEmpty ? Text(name.isNotEmpty ? name[0].toUpperCase() : 'S', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: Colors.teal)) : null),
            const SizedBox(width: 14),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(color: Colors.blue.shade50, borderRadius: BorderRadius.circular(20)),
