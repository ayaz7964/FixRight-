...
class _JobDetailPageState extends State<_JobDetailPage> {
  ...
  void _showEditDialog() {
    final tCtrl = TextEditingController(
      text: _jobData['title'] as String? ?? '',
    );
    final dCtrl = TextEditingController(
      text: _jobData['description'] as String? ?? '',
    );
  ...
  Widget build(BuildContext context) {
    ...
    final isInsured =
        (_jobData['orderType'] as String? ?? 'simple') == 'insured';
        ...
        child: Column(
          ...
          children: [
            ...
            if (status == 'in_progress') ...[
              ...
              _AcceptedSellerCard(
                jobId: widget.jobId,
                acceptedBidder: _jobData['acceptedBidder'] as String?,
              ),
              if (isInsured) ...[
                const SizedBox(height: 12),
                _InsuranceActiveBanner(),
              ],
            ],
            if (status == 'completed' || status == 'expert_completed')
...
class _InsurancePaymentSectionState extends State<_InsurancePaymentSection> {
      ...
      if (url != null && url.isNotEmpty) {
        ...
        if (sellerId.isNotEmpty)
          await NotificationService.send(
            ...
            body: 'Buyer submitted receipt. Waiting admin verification.',
  ...
  Widget build(BuildContext context) {
    ...
    final rejectionReason =
        widget.jobData['paymentRejectionReason'] as String? ?? '';
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Please fix the issue above and upload a new, clear receipt.',
                        style: TextStyle(
                          fontSize: 12,
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              Text(
                'Transfer PKR ${totalAmount.toStringAsFixed(0)} to the company account, then upload your receipt below.',
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      isResubmit
                          ? 'Address the rejection reason and upload a clear, correct receipt.'
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          Text(
            'Your receipt has been submitted. Admin will verify and activate your order within a few hours.',
...
class _CompletedJobSectionState extends State<_CompletedJobSection> {
  bool _loading = false;
  bool get _isInsured =>
      (widget.jobData['orderType'] as String? ?? 'simple') == 'insured';
  bool get _canClaim {
    ...
    try {
      ...
      if (_sellerUid.isNotEmpty)
        await NotificationService.send(
          ...
          body: 'Buyer is satisfied. Admin will release your earnings soon.',
          type: 'job_completed',
          jobId: widget.jobId,
        );
      if (mounted) {
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                    'Are you satisfied? Confirm to notify admin to release payment.',
                    style: TextStyle(
                      fontSize: 12,
                      color: Colors.green.shade800,
                    ),
...
class _HistoryJobCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final buyerFeedback =
        jobData['buyerFeedback'] as String? ?? ''; // ✅ Fixed null cast
...
class _BidCardState extends State<_BidCard> {
  ...
  Future<void> _openConversation(BuildContext ctx) async {
    ...
    if (!(await db.collection('conversations').doc(convId).get()).exists) {
      ...
      final buyerData = buyerDoc.data() ?? {};
      final buyerName =
          '${buyerData['firstName'] as String? ?? ''} ${buyerData['lastName'] as String? ?? ''}'
              .trim();
      await db.collection('conversations').doc(convId).set({
        'participantIds': [widget.buyerUid, sellerId],
        ...
        'participantProfileImages': {
          widget.buyerUid: buyerData['profileImage'] as String? ?? '',
        ...
        'relatedJobTitle': widget.jobData['title'] as String? ?? '',
      });
    }
          ...
          children: [
            ...
            FutureBuilder<Map<String, dynamic>>(
              ...
              builder: (ctx, snap) {
                ...
                if (!info['isEligible'])
                  return _ic(
                    '⚠️ Balance too low (PKR ${(info['freeBalance'] as double).toStringAsFixed(0)} free)',
                    Colors.red,
                  );
                if (info['isFreeOrder'] as bool)
                  return _ic(
                    '🎁 Free order — ${info['freeOrdersLeft']} free left',
                    Colors.green,
                  );
                return _ic(
                  '${((info['rate'] as double) * 100).toStringAsFixed(0)}% commission (PKR ${(info['commissionAmount'] as double).toStringAsFixed(0)}) from seller',
                  Colors.orange,
                );
              },
            ),
    ...
    try {
      ...
      if (!bidIsFree && storedCommission > 0) {
        ...
        batch.set(commRef, {
          ...
          'commissionAmount': storedCommission,
          'commissionRate': storedRate,
          'orderAmount': _proposedAmount,
          'orderType': _wantsInsurance ? 'insured' : 'simple',
          'city': buyerCity,
          'createdAt': FieldValue.serverTimestamp(),
        });
        batch.set(db.collection('admin_earnings').doc('summary'), {
          'totalCommission': FieldValue.increment(storedCommission),
      ...
      batch.set(orderRef, {
        ...
        'jobId': widget.jobId,
        'jobTitle': widget.jobData['title'] as String? ?? '',
        'jobDescription': widget.jobData['description'] as String? ?? '',
        'jobLocation': widget.jobData['location'] as String? ?? '',
      ...
      for (final d in otherBids.docs) {
        final rId =
            (d.data() as Map)['sellerId'] as String? ?? ''; // ✅ Fixed null cast
        if (rId.isNotEmpty)
          await NotificationService.send(
            toUid: rId,
                      ...
                      child: Column(
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              '⚠️ After payment, upload your receipt. Order activates after admin verification. Admin releases payment to seller after job completion.',
                              style: TextStyle(
                                fontSize: 11,
                                color: Colors.orange.shade800,
                              ),
