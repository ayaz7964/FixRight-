...
class _JobDetailScreenState extends State<_JobDetailScreen> {
  bool _alreadyBid = false, _checkingBid = true;
  Map<String,dynamic>? _sellerInfo;
  Map<String,dynamic>? _ownBidData; // ← stores own bid details for display/edit
  ...
  Future<void> _loadData() async {
    try {
      ...
      if (mounted) setState(() {
        _alreadyBid  = bidDoc.exists;
        _ownBidData  = bidDoc.exists ? bidDoc.data() as Map<String,dynamic> : null;
        _sellerInfo  = {
          'isFree': isFree, 'freeLeft': isFree ? kSellerFreeOrderLimit - jobsCompleted : 0,
          'balance': balance, 'reserved': reserved, 'freeBalance': freeBalance,
          'commissionRequired': commissionRequired, 'rate': rate,
  ...
  void _refreshOwnBid() async {
    try {
      final bidDoc = await FirebaseFirestore.instance.collection('jobs').doc(widget.jobId).collection('bids').doc(widget.sellerUid).get();
      if (mounted) setState(() => _ownBidData = bidDoc.exists ? bidDoc.data() as Map<String,dynamic> : null);
    } catch (_) {}
  }
  ...
  Widget _buildOwnBidSection() {
    ...
    switch (status) {
      case 'accepted': sc = Colors.green; si = Icons.check_circle; sl = 'ACCEPTED'; break;
      case 'rejected': sc = Colors.red; si = Icons.cancel; sl = 'REJECTED'; break;
      default: sc = Colors.orange; si = Icons.hourglass_empty; sl = 'PENDING';
    }
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 8),
            Text('Your Bid', style: TextStyle(fontWeight: FontWeight.bold, color: sc, fontSize: 14)),
            const Spacer(),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
              decoration: BoxDecoration(color: sc.withOpacity(0.15), borderRadius: BorderRadius.circular(20)),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(si, size: 12, color: sc), const SizedBox(width: 4),
                Text(sl, style: TextStyle(color: sc, fontWeight: FontWeight.bold, fontSize: 11)),
              ]),
            ),
          ]),
        ),
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Bid Amount', style: TextStyle(fontSize: 11, color: Colors.grey[500])),
              ...
              if (createdAt != null)
                Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                  Text('Submitted', style: TextStyle(fontSize: 10, color: Colors.grey[400])),
                  Text(DateFormat('dd MMM, hh:mm a').format(createdAt.toDate()),
            ...
            // Commission info
            if (isFreeOrder)
              Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.green.shade200)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.card_giftcard, size: 12, color: Colors.green.shade700), const SizedBox(width: 4), Text('Free order — no commission', style: TextStyle(fontSize: 11, color: Colors.green.shade700, fontWeight: FontWeight.w600))]))
            else if (commReserved > 0)
              Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(color: Colors.orange.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.orange.shade200)), child: Text('${(commRate*100).toStringAsFixed(0)}% commission reserved: PKR ${commReserved.toStringAsFixed(0)}', style: TextStyle(fontSize: 11, color: Colors.orange.shade700, fontWeight: FontWeight.w600))),
            // Proposal
            if (proposal.isNotEmpty) ...[
              ...
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Your Proposal', style: TextStyle(fontSize: 11, color: Colors.grey[500], fontWeight: FontWeight.w600)),
                  const SizedBox(height: 4),
                  Text(proposal, style: const TextStyle(fontSize: 13, height: 1.4)),
                ]),
              ),
            ],
            const SizedBox(height: 14),
            ...
            if (status == 'pending')
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  ...
                  label: const Text('Edit My Bid', style: TextStyle(fontWeight: FontWeight.w600)),
              ...
              Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.green.shade200)), child: Row(children: [Icon(Icons.celebration, color: Colors.green.shade600, size: 16), const SizedBox(width: 8), Expanded(child: Text('🎉 Bid accepted! Get ready to start the job.', style: TextStyle(color: Colors.green.shade700, fontSize: 12, fontWeight: FontWeight.w600)))]))
            else if (status == 'rejected')
              Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.red.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.red.shade200)), child: Row(children: [Icon(Icons.info_outline, color: Colors.red.shade600, size: 16), const SizedBox(width: 8), Expanded(child: Text('Buyer chose another seller. Keep bidding on other jobs!', style: TextStyle(color: Colors.red.shade700, fontSize: 12)))])),
          ]),
        ),
      ]),
    );
  ...
  Widget _buildPlaceBidSection(double budget, bool isInsured) {
    ...
    if (!isEligible) return SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: null, icon: const Icon(Icons.block), label: const Text('Cannot Bid — Add Funds First'), style: OutlinedButton.styleFrom(foregroundColor: Colors.grey, side: const BorderSide(color: Colors.grey), padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)))));
    return SizedBox(width: double.infinity, child: ElevatedButton.icon(
      onPressed: () => showModalBottomSheet(context: context, isScrollControlled: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (_) => _PlaceBidSheet(jobId: widget.jobId, jobData: widget.jobData, sellerUid: widget.sellerUid, sellerInfo: _sellerInfo!, onBidPlaced: () { setState(() => _alreadyBid = true); _loadData(); })),
      icon: const Icon(Icons.gavel), label: const Text('Place a Bid', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      style: ElevatedButton.styleFrom(backgroundColor: Colors.green.shade700, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
    ));
  }
      ...
      builder: (ctx, snap) {
        ...
        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            const SizedBox(width: 8),
            Text('All Bids on This Job', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.grey[800])),
            const SizedBox(width: 8),
            Container(
              ...
              child: Text('${docs.length}', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.orange.shade700, fontSize: 12)),
            ),
          ]),
          const SizedBox(height: 10),
          ...docs.map((doc) {
...
class _BidRowCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Color sc;
    switch (status) {
      case 'accepted': sc = Colors.green; break;
      case 'rejected': sc = Colors.red.shade300; break;
      default: sc = Colors.orange;
    }
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            CircleAvatar(
              radius: isOwn ? 22 : 18,
              backgroundColor: isOwn ? Colors.green.shade100 : Colors.grey.shade100,
              backgroundImage: sellerImage.isNotEmpty ? NetworkImage(sellerImage) : null,
              child: sellerImage.isEmpty
                  ? Text(sellerName.isNotEmpty ? sellerName[0].toUpperCase() : '?',
                      style: TextStyle(fontWeight: FontWeight.bold, color: isOwn ? Colors.green.shade700 : Colors.grey[600], fontSize: isOwn ? 16 : 13))
                  : null,
            ),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text(
                  ...
                  style: TextStyle(fontWeight: FontWeight.bold, fontSize: isOwn ? 14 : 13, color: isOwn ? Colors.green.shade800 : Colors.black87),
                )),
                if (isOwn) Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2), decoration: BoxDecoration(color: Colors.green.shade600, borderRadius: BorderRadius.circular(8)), child: const Text('YOU', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))),
              ]),
              if (rating > 0)
                Row(children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              ...
              Container(
                ...
                child: Text(status.toUpperCase(), style: TextStyle(fontSize: 9, fontWeight: FontWeight.bold, color: sc)),
              ),
            ]),
          ]),
          ...
          if (isOwn) ...[
            ...
            isFreeOrder
                ...
                : _chip('${(commRate*100).toStringAsFixed(0)}% comm reserved: PKR ${commReserved.toStringAsFixed(0)}', Colors.orange),
          ],

          // Skills (compact)
          if (skills.isNotEmpty) ...[
            const SizedBox(height: 6),
            Wrap(spacing: 4, runSpacing: 3, children: skills.take(3).map((s) => Container(
              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
              decoration: BoxDecoration(
                color: isOwn ? Colors.green.shade100 : Colors.grey.shade100,
                borderRadius: BorderRadius.circular(6),
              ),
              child: Text(s, style: TextStyle(fontSize: 10, color: isOwn ? Colors.green.shade700 : Colors.grey[700])),
            )).toList()),
          ],
          ...
          if (proposal.isNotEmpty) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: isOwn ? Colors.green.shade200 : Colors.grey.shade200),
              ),
              child: Text(
                proposal,
          ...
          if (isOwn && status == 'pending' && sellerInfo != null) ...[
            ...
            SizedBox(
              ...
              child: OutlinedButton.icon(
                ...
                label: const Text('Edit Bid', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                style: OutlinedButton.styleFrom(
                  foregroundColor: Colors.green.shade700,
  ...
  Widget _chip(String t, Color c) => Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(color: c.withOpacity(0.1), borderRadius: BorderRadius.circular(6), border: Border.all(color: c.withOpacity(0.3))), child: Text(t, style: TextStyle(fontSize: 10, color: c, fontWeight: FontWeight.w600)));
}
...
class _EditBidSheet extends StatefulWidget {
  ...
  @override State<_EditBidSheet> createState() => _EditBidSheetState();
}
...
class _EditBidSheetState extends State<_EditBidSheet> {
  ...
  void initState() {
    super.initState();
    final existingAmount = (widget.existingBid['proposedAmount'] ?? 0).toDouble();
    _amountCtrl   = TextEditingController(text: existingAmount.toStringAsFixed(0));
    _proposalCtrl = TextEditingController(text: widget.existingBid['proposal'] as String? ?? '');
  ...
  Future<void> _submitEdit() async {
    ...
    try {
      ...
      final budget    = (widget.jobData['budget'] ?? 0).toDouble();
      if (newAmount > budget) {
        _snack('Bid cannot exceed PKR ${budget.toStringAsFixed(0)}', Colors.red);
      ...
      final oldCommission  = (widget.existingBid['commissionReserved'] ?? 0).toDouble();
      final delta          = newCommission - oldCommission; // positive = need more, negative = release some
      final freeBalance    = balance - reserved;

      // Only check if we need MORE commission than before
      if (!isFree && delta > 0 && freeBalance < delta) {
        _snack('Insufficient free balance. Need PKR ${delta.toStringAsFixed(0)} more.', Colors.red);
        setState(() => _isSubmitting = false);
        return;
      }
      ...
      batch.update(db.collection('jobs').doc(widget.jobId).collection('bids').doc(widget.sellerUid), updatedPayload);
      // Update bid in seller's myBids subcollection
      batch.update(db.collection('sellers').doc(widget.sellerUid).collection('myBids').doc(widget.jobId), updatedPayload);
      ...
      widget.onBidUpdated();
      Navigator.pop(context);
      _snack('✅ Bid updated to PKR ${newAmount.toStringAsFixed(0)}', Colors.green);
    } catch (e) {
        ...
        child: Form(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              const SizedBox(width: 8),
              const Text('Edit Your Bid', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              const Spacer(),
            ...
            Container(
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 8),
                Text('Previous bid: PKR ${oldAmount.toStringAsFixed(0)}', style: TextStyle(fontSize: 12, color: Colors.blue.shade800, fontWeight: FontWeight.w600)),
                const Spacer(),
                Text('Max: PKR ${budget.toStringAsFixed(0)}', style: TextStyle(fontSize: 11, color: Colors.blue.shade700)),
              ]),
            ),
            const SizedBox(height: 10),
            ...
            if (!isFree)
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Icon(Icons.account_balance_wallet, size: 14, color: Colors.green.shade700),
                    const SizedBox(width: 6),
                    Text('Wallet', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.green.shade700)),
                    const Spacer(),
                    Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)), child: Text('${(rate*100).toStringAsFixed(0)}% commission', style: TextStyle(fontSize: 10, color: Colors.grey[600]))),
                  ]),
                  const SizedBox(height: 6),
                  _bRow('Wallet', 'PKR ${balance.toStringAsFixed(0)}', Colors.teal),
                  if (reserved > 0) _bRow('Reserved (all bids)', '−PKR ${reserved.toStringAsFixed(0)}', Colors.orange),
                  _bRow('Free Balance', 'PKR ${freeBalance.toStringAsFixed(0)}', Colors.green),
                  if (oldCommission > 0) _bRow('Already reserved for this bid', 'PKR ${oldCommission.toStringAsFixed(0)}', Colors.blue),
                  if (_liveCommission != null) ...[
                    const Divider(height: 10),
                    _bRow('New commission for this bid', 'PKR ${_liveCommission!.toStringAsFixed(0)}', Colors.orange),
                    _bRow('Commission adjustment', '${delta >= 0 ? '+' : ''}PKR ${delta.toStringAsFixed(0)}', delta > 0 ? Colors.red : Colors.green),
                    _bRow('Free balance after update', 'PKR ${(freeBalance - delta).toStringAsFixed(0)}', (freeBalance - delta) < 0 ? Colors.red : Colors.green),
                  ],
                ]),
              ),
            if (isFree)
            ...
            TextFormField(
              ...
              validator: (v) {
                ...
                if (amt > budget) return 'Cannot exceed PKR ${budget.toStringAsFixed(0)}';
                return null;
              },
            ),
            const SizedBox(height: 14),
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(_isSubmitting ? 'Updating...' : 'Update Bid', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
