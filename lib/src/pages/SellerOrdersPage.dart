...
class _SellerOrdersPageState extends State<SellerOrdersPage>
  ...
  late String _uid;
  String _sellerCity = '';  // ✅ seller's own city
  ...
  // ✅ Load seller's own city from users collection
  ...
  Future<void> _checkBalanceAndNotify() async {
    ...
    try {
      ...
      if (jobsCompleted >= kSellerFreeOrderLimit && balance < 100) {
        WidgetsBinding.instance.addPostFrameCallback((_) { if (mounted) _showLowBalanceBanner(balance); });
      }
    } catch (_) {}
  }
  ...
  void _showLowBalanceBanner(double balance) {
    ScaffoldMessenger.of(context).showMaterialBanner(MaterialBanner(
      ...
      content: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Icon(Icons.warning_amber_rounded, color: Colors.red.shade700, size: 18), const SizedBox(width: 8), Text('Low Balance — Orders Paused', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red.shade700, fontSize: 13))]),
      ...
      actions: [
        ...
        ElevatedButton(onPressed: () => ScaffoldMessenger.of(context).hideCurrentMaterialBanner(), style: ElevatedButton.styleFrom(backgroundColor: Colors.red.shade700, foregroundColor: Colors.white), child: const Text('ADD FUNDS')),
      ],
    ));
  }
  ...
  Widget build(BuildContext context) {
    if (_uid.isEmpty) {
      return Scaffold(appBar: AppBar(title: const Text('Find Jobs'), backgroundColor: Colors.green.shade700, foregroundColor: Colors.white), body: const Center(child: Text('Could not identify user. Please log in again.')));
    }
    return Scaffold(
...
// ─────────────────────────────────────────────────────────────
// Balance Bar — shows Earning AND Available_Balance
// ─────────────────────────────────────────────────────────────
...
// ═══════════════════════════════════════════════════════════════
//  BROWSE OPEN JOBS — city priority + all-city toggle
// ═══════════════════════════════════════════════════════════════
            ...
            Row(children: [
              ...
              Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.my_location, size: 13, color: Colors.green.shade700), const SizedBox(width: 4),
                  Text('Your city: ${widget.sellerCity}', style: TextStyle(fontSize: 12, color: Colors.green.shade700, fontWeight: FontWeight.w600)),
                ]),
              ),
              const Spacer(),
              GestureDetector(
                ...
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                  decoration: BoxDecoration(color: _showAllCities ? Colors.blue.shade50 : Colors.grey.shade100, borderRadius: BorderRadius.circular(20), border: Border.all(color: _showAllCities ? Colors.blue.shade300 : Colors.grey.shade300)),
      ...
      Expanded(
        child: StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snap) {
            ...
            // ✅ City-based priority sort
            if (widget.sellerCity.isNotEmpty && !_showAllCities) {
              final myCity = widget.sellerCity.toLowerCase().trim();
              final sameCityDocs = allDocs.where((d) => ((d.data() as Map)['city'] as String? ?? '').toLowerCase().trim() == myCity).toList();
              final otherCityDocs = allDocs.where((d) => ((d.data() as Map)['city'] as String? ?? '').toLowerCase().trim() != myCity).toList();
              for (final group in [sameCityDocs, otherCityDocs]) { group.sort((a, b) { final aT = (a.data() as Map)['postedAt'] as Timestamp?; final bT = (b.data() as Map)['postedAt'] as Timestamp?; if (aT == null || bT == null) return 0; return bT.compareTo(aT); }); }
            ...
            return ListView.builder(
              padding: const EdgeInsets.all(12), itemCount: allDocs.length,
              itemBuilder: (ctx, i) {
                final data = allDocs[i].data() as Map<String, dynamic>;
                final jobCity = (data['city'] as String? ?? '').toLowerCase().trim();
                final isSameCity = widget.sellerCity.isNotEmpty && jobCity == widget.sellerCity.toLowerCase().trim();
...
// ─────────────────────────────────────────────────────────────
// Open Job Card — city display + TTS
// ─────────────────────────────────────────────────────────────
class _OpenJobCard extends StatelessWidget {
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Container(
            ...
            decoration: BoxDecoration(color: isSameCity ? Colors.green.shade50 : isInsured ? Colors.blue.shade50 : Colors.grey.shade50, borderRadius: const BorderRadius.vertical(top: Radius.circular(14))),
            child: Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: [
                  ...
                  // ✅ Show job city vs seller city
                  if (jobCity.isNotEmpty) ...[
                    const SizedBox(width: 8),
                    Icon(Icons.location_city, size: 12, color: isSameCity ? Colors.green.shade600 : Colors.grey[400]),
                    const SizedBox(width: 3),
                    Text(jobCity, style: TextStyle(fontSize: 12, fontWeight: isSameCity ? FontWeight.bold : FontWeight.normal, color: isSameCity ? Colors.green.shade700 : Colors.grey[500])),
          ...
          Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (description.isNotEmpty) ...[
              ...
              // ✅ TranslatedText for job description
              TranslatedText(text: description, contentId: 'open_$jobId', style: TextStyle(fontSize: 12, color: Colors.grey[700], height: 1.4), maxLines: 2, showListenButton: false),
            ],
            const SizedBox(height: 8),
  ...
  Widget _insuredBadge() => Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.blue, borderRadius: BorderRadius.circular(8)), child: Row(mainAxisSize: MainAxisSize.min, children: const [Icon(Icons.shield, size: 10, color: Colors.white), SizedBox(width: 3), Text('INSURED', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))]));
}

// ═══════════════════════════════════════════════════════════════
//  JOB DETAIL SCREEN — with TTS
// ═══════════════════════════════════════════════════════════════
class _JobDetailScreen extends StatefulWidget {
  final String jobId; final Map<String, dynamic> jobData; final String sellerUid;
...
class _JobDetailScreenState extends State<_JobDetailScreen> {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Card(elevation: 2, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)), child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            // ✅ TTS Listen row right under title
            const SizedBox(height: 8),
            ...
            if (location.isNotEmpty) ...[_infoRow(Icons.location_on, 'Address', location), const SizedBox(height: 8)],
  ...
  Widget _buildEligibilityCard() {
    ...
    if (!isEligible) return Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.red.shade50, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.red.shade200)), child: Row(children: [Icon(Icons.warning_amber_rounded, color: Colors.red.shade700, size: 20), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Insufficient Balance', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red.shade700)), Text('Need min PKR ${kSellerMinBalance.toStringAsFixed(0)}. Current: PKR ${balance.toStringAsFixed(0)}', style: TextStyle(fontSize: 12, color: Colors.red.shade800))]))]));
    if (isFree) return Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.green.shade200)), child: Row(children: [Icon(Icons.card_giftcard, color: Colors.green.shade700, size: 20), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('🎁 Free Order', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green.shade700)), Text('$freeLeft free order(s) remaining — No commission', style: TextStyle(fontSize: 12, color: Colors.green.shade800))]))]));
    return Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.orange.shade50, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.orange.shade200)), child: Row(children: [Icon(Icons.percent, color: Colors.orange.shade700, size: 20), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('10% Commission Per Order', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.orange.shade700)), Text('Deducted when buyer accepts. Balance: PKR ${balance.toStringAsFixed(0)}', style: TextStyle(fontSize: 12, color: Colors.orange.shade800))]))]));
  }
  ...
  Widget _infoRow(IconData icon, String label, String value) => Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon, size: 16, color: Colors.grey[500]), const SizedBox(width: 8), Text('$label: ', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)), Expanded(child: Text(value, style: TextStyle(fontSize: 13, color: Colors.grey[700])))]);
}

// ═══════════════════════════════════════════════════════════════
//  PLACE BID SHEET — cash note + TTS
// ═══════════════════════════════════════════════════════════════
class _PlaceBidSheet extends StatefulWidget {
  final String jobId; final Map<String, dynamic> jobData; final String sellerUid; final Map<String, dynamic> sellerInfo; final VoidCallback onBidPlaced;
...
class _PlaceBidSheetState extends State<_PlaceBidSheet> {
  ...
  Future<void> _submitBid() async {
    ...
    try {
      ...
      final bidPayload = {'sellerId': widget.sellerUid, 'sellerName': sellerName, 'sellerImage': sellerImage, 'rating': rating, 'skills': skills, 'jobsCompleted': jobsCompleted, 'proposedAmount': amount, 'proposal': _proposalCtrl.text.trim(), 'status': 'pending', 'jobId': widget.jobId, 'jobTitle': widget.jobData['title'] ?? '', 'jobBudget': budget, 'posterName': widget.jobData['posterName'] ?? '', 'isInsured': (widget.jobData['orderType'] ?? 'simple') == 'insured', 'createdAt': FieldValue.serverTimestamp()};
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Container(padding: const EdgeInsets.all(20), child: Form(key: _formKey, child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (!isInsured) Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.green.shade200)),
          child: Row(children: [Icon(Icons.payments_outlined, size: 14, color: Colors.green.shade700), const SizedBox(width: 8), Expanded(child: Text('💵 Cash on Delivery — You will collect payment directly from the buyer after completing the job. This amount does NOT go to your wallet.', style: TextStyle(fontSize: 11, color: Colors.green.shade800, fontWeight: FontWeight.w500)))])),
        if (isInsured) Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: isFree ? Colors.green.shade50 : Colors.orange.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: isFree ? Colors.green.shade200 : Colors.orange.shade200)),
          child: Row(children: [Icon(isFree ? Icons.card_giftcard : Icons.percent, size: 14, color: isFree ? Colors.green.shade700 : Colors.orange.shade700), const SizedBox(width: 8), Expanded(child: Text(isFree ? '🎁 Free order ($freeLeft free left) — No commission' : '10% deducted on acceptance • Balance: PKR ${balance.toStringAsFixed(0)}', style: TextStyle(fontSize: 12, color: isFree ? Colors.green.shade800 : Colors.orange.shade800)))])),
        const SizedBox(height: 14),
...
class _MyBidCard extends StatelessWidget {
  final Map<String, dynamic> bid; final String jobId; final String sellerUid;
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, bidSnap) {
        ...
        switch (status) { case 'accepted': statusColor = Colors.green; statusIcon = Icons.check_circle; break; case 'rejected': statusColor = Colors.red; statusIcon = Icons.cancel; break; default: statusColor = Colors.orange; statusIcon = Icons.hourglass_empty; }

        return Container(
          margin: const EdgeInsets.only(bottom: 12),
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: status == 'accepted' ? Colors.green.shade200 : status == 'rejected' ? Colors.red.shade100 : Colors.grey.shade200), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 6)]),
          child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 10),
            if (status == 'accepted')
              Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.green.shade200)), child: Row(children: [Icon(Icons.celebration, color: Colors.green.shade600, size: 16), const SizedBox(width: 8), Expanded(child: Text(isInsured == true ? '🎉 Accepted! Wait for buyer to pay company before starting.' : '🎉 Accepted! Collect cash on job completion.', style: TextStyle(color: Colors.green.shade700, fontSize: 12, fontWeight: FontWeight.w600)))]))
      ...
      builder: (context, snap) {
        ...
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, expertSnap) {
            final expertJobs = (expertSnap.data?.docs ?? []).where((d) => (d.data() as Map)['status'] == 'expert_assigned').toList();
            ...
            return ListView.builder(padding: const EdgeInsets.all(12), itemCount: combined.length, itemBuilder: (ctx, i) {
...
class _ActiveJobCard extends StatefulWidget {
  final String jobId; final Map<String, dynamic> jobData; final String sellerUid; final bool isExpertRole;
...
class _ActiveJobCardState extends State<_ActiveJobCard> {
  ...
  Future<void> _markCompleted() async {
    ...
    try {
      ...
      // ✅ KEY FIX: Earning always incremented to show total earned
      //    Available_Balance NOT updated here:
      //    - Cash order: buyer pays physically, no wallet change needed
      //    - Insured: admin releases after claim window via admin panel
      batch.update(db.collection('sellers').doc(widget.sellerUid), {
        ...
        'Earning': FieldValue.increment(acceptedAmount), // ✅ Net earnings tracker
        // Available_Balance left untouched intentionally
      });
      ...
      if (buyerUid.isNotEmpty) {
        await NotificationService.send(toUid: buyerUid,
          ...
          body: _isInsured ? 'Your job "${widget.jobData['title']}" is complete. You have 3 days to accept or claim insurance.' : 'Job "${widget.jobData['title']}" is done. Cash payment goes to the worker directly.',
          type: 'job_completed', jobId: widget.jobId);
      }

      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(_isInsured ? '✅ Job done! Buyer has 3 days to claim. Earnings released after.' : '🎉 Job done! Collect PKR ${acceptedAmount.toStringAsFixed(0)} cash from buyer. Earnings recorded.'),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: headerColor, borderRadius: const BorderRadius.vertical(top: Radius.circular(14))),
          child: Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text(title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold))),
                Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3), decoration: BoxDecoration(color: statusLabelColor.withOpacity(0.15), borderRadius: BorderRadius.circular(8)), child: Text(statusLabel, style: TextStyle(color: statusLabelColor, fontSize: 9, fontWeight: FontWeight.bold))),
                if (_isInsured) ...[const SizedBox(width: 4), Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 2), decoration: BoxDecoration(color: Colors.blue, borderRadius: BorderRadius.circular(8)), child: Row(mainAxisSize: MainAxisSize.min, children: const [Icon(Icons.shield, size: 9, color: Colors.white), SizedBox(width: 2), Text('INS', style: TextStyle(color: Colors.white, fontSize: 8, fontWeight: FontWeight.bold))]))],
              ]),
              Row(children: [
                Text('Client: $posterName', style: TextStyle(fontSize: 12, color: Colors.grey[600])),
                if (city.isNotEmpty) ...[const SizedBox(width: 8), Icon(Icons.location_city, size: 12, color: Colors.grey[400]), const SizedBox(width: 3), Text(city, style: TextStyle(fontSize: 11, color: Colors.grey[500]))],
              ]),
            ])),
            const SizedBox(width: 8),
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
        ...
        Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: _isInsured ? Colors.blue.shade50 : Colors.green.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: _isInsured ? Colors.blue.shade200 : Colors.green.shade200)),
            child: Row(children: [Icon(_isInsured ? Icons.lock_outline : Icons.payments_outlined, size: 14, color: _isInsured ? Colors.blue.shade700 : Colors.green.shade700), const SizedBox(width: 8), Expanded(child: Text(_isInsured ? 'Payment locked. Released after 3-day claim window. Earnings counted after release.' : '💵 Collect PKR ${acceptedAmount.toStringAsFixed(0)} cash from client directly.', style: TextStyle(fontSize: 11, color: _isInsured ? Colors.blue.shade800 : Colors.green.shade800)))])),
          const SizedBox(height: 8),
...
// ═══════════════════════════════════════════════════════════════
//  SELLER HISTORY TAB — with rating display + bank details
// ═══════════════════════════════════════════════════════════════
class _SellerHistoryTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, sellerSnap) {
        ...
        final rating = (sellerData['Rating'] ?? 0.0).toDouble();
          ...
          builder: (context, snap) {
            return StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, expertSnap) {
                ...
                for (final d in [...(snap.data?.docs ?? []), ...(expertSnap.data?.docs ?? [])]) { if (allIds.add(d.id)) allDocs.add(d); }
                allDocs.sort((a, b) { final aT = (a.data() as Map)['postedAt'] as Timestamp?; final bT = (b.data() as Map)['postedAt'] as Timestamp?; if (aT == null || bT == null) return 0; return bT.compareTo(aT); });
                ...
                return ListView(padding: const EdgeInsets.all(12), children: [
                  ...
                  Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Row(children: [
                        _earningsTile('Wallet', 'PKR ${balance.toStringAsFixed(0)}', Icons.wallet),
                        _earningsTile('Rating', '${rating.toStringAsFixed(1)} ⭐', Icons.star),
                        _earningsTile('Done', '$completedJobs', Icons.task_alt),
                        _earningsTile('Active', '$activeJobs', Icons.autorenew),
                      ]),
                      const SizedBox(height: 8),
                      Row(children: [_earningsTile('Insured', '$insuredJobs', Icons.shield_outlined), _earningsTile('Cash', '$cashJobs', Icons.payments_outlined)]),
                      const SizedBox(height: 8),
                      // ✅ Note about cash vs balance
                      Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.white.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
                        child: const Text('💡 Cash orders: collected directly from buyer. Insurance orders: paid to your wallet after admin releases.', style: TextStyle(fontSize: 11, color: Colors.white70))),
                    ]),
                  ),
  ...
  Widget _earningsTile(String label, String value, IconData icon) => Expanded(child: Column(children: [Icon(icon, color: Colors.white60, size: 16), const SizedBox(height: 2), Text(value, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 11)), Text(label, style: const TextStyle(color: Colors.white54, fontSize: 9))]));
}
...
class _BankDetailsSectionState extends State<_BankDetailsSection> {
  bool _hasBankDetails = false; bool _loading = true; bool _hasInsuredCompleted = false;
...
// ─────────────────────────────────────────────────────────────
// Seller History Card — with buyer rating display
// ─────────────────────────────────────────────────────────────
class _SellerHistoryCard extends StatelessWidget {
  final Map<String, dynamic> jobData; final String jobId; final String sellerUid;
  ...
  Widget build(BuildContext context) {
    ...
    switch (status) { case 'completed': case 'expert_completed': statusColor = Colors.green; statusLabel = 'Completed'; break; case 'in_progress': statusColor = Colors.blue; statusLabel = 'In Progress'; break; case 'claim_pending': statusColor = Colors.red; statusLabel = 'Claim Filed'; break; case 'expert_assigned': statusColor = Colors.purple; statusLabel = 'Expert Sent'; break; case 'cancelled': statusColor = Colors.red; statusLabel = 'Cancelled'; break; default: statusColor = Colors.grey; statusLabel = status; }
    ...
    return Container(
      ...
      child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Expanded(child: Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15))), Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(color: statusColor.withOpacity(0.1), borderRadius: BorderRadius.circular(20)), child: Text(statusLabel, style: TextStyle(color: statusColor, fontWeight: FontWeight.bold, fontSize: 11)))]),
        const SizedBox(height: 4),
        // ✅ City
        ...
        Row(children: [
          ...
          const SizedBox(width: 6), _chip('PKR ${acceptedAmount.toStringAsFixed(0)}', Colors.teal),
        ...
        Row(children: [
          ...
          if (paymentStatus == 'locked') Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(color: Colors.orange.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.orange.shade200)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.lock_outline, size: 12, color: Colors.orange.shade700), const SizedBox(width: 3), Text('Pending', style: TextStyle(fontSize: 11, color: Colors.orange.shade700, fontWeight: FontWeight.bold))])),
        ]),
        // ✅ Buyer rating received
        if (buyerRating > 0) ...[
          const Divider(height: 14),
          Row(children: [
            ...List.generate(5, (i) => Icon(i < buyerRating ? Icons.star_rounded : Icons.star_outline_rounded, size: 18, color: Colors.amber)),
            const SizedBox(width: 8),
            Text('$buyerRating/5 from client', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
          ]),
